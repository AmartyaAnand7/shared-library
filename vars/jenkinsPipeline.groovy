    ...
    pipeline {
	   ...
	   stage("Sonar Report") {
		   ...
		   steps {
			dir ("${directoryName}") {
				sh "pwd"
				sh "mvn clean install -DskipTests=true sonar:sonar -Dsonar.host.url=http://16.107.50.87:8090 -Dsonar.exclusions=**/*.ts -Dsonar.analysis.mode=publish -Dsonar.projectName= ${projectName}"
			}
		  }
	   }
       }
